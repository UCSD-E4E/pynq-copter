...
entity rpy_to_duty_AXILiteS_s_axi is
generic (
...
port (
    ...
    regs_in_V_we0         :in   STD_LOGIC;
    regs_in_V_d0          :in   STD_LOGIC_VECTOR(7 downto 0)
);
end entity rpy_to_duty_AXILiteS_s_axi;
...
architecture behave of rpy_to_duty_AXILiteS_s_axi is
    ...
    signal int_regs_in_V_d1    : UNSIGNED(31 downto 0);
    signal int_regs_in_V_q1    : UNSIGNED(31 downto 0);
    signal int_regs_in_V_read  : STD_LOGIC;
    signal int_regs_in_V_write : STD_LOGIC;
    signal int_regs_in_V_shift : UNSIGNED(1 downto 0);

    component rpy_to_duty_AXILiteS_s_axi_ram is
        generic (
            BYTES   : INTEGER :=4;
            DEPTH   : INTEGER :=256;
            AWIDTH  : INTEGER :=8);
    ...
    end component rpy_to_duty_AXILiteS_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_regs_in_V
int_regs_in_V : rpy_to_duty_AXILiteS_s_axi_ram
generic map (
     BYTES    => 4,
     DEPTH    => 2,
     AWIDTH   => log2(2))
port map (
     ...
     ce0      => int_regs_in_V_ce0,
     we0      => int_regs_in_V_we0,
     be0      => int_regs_in_V_be0,
     d0       => int_regs_in_V_d0,
     q0       => int_regs_in_V_q0,
     clk1     => ACLK,
     address1 => int_regs_in_V_address1,
     ce1      => int_regs_in_V_ce1,
     we1      => int_regs_in_V_we1,
     be1      => int_regs_in_V_be1,
     d1       => int_regs_in_V_d1,
     q1       => int_regs_in_V_q1);
...
-- ----------------------- AXI READ ----------------------
    ...
    RRESP   <= "00";  -- OKAY
    RVALID_t  <= '1' when (rstate = rddata) and (int_regs_in_V_read = '0') else '0';
    RVALID    <= RVALID_t;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                ...
                elsif (int_regs_in_V_read = '1') then
                    rdata_data <= int_regs_in_V_q1;
                end if;
            end if;
        end if;
    end process;
...
-- ----------------------- Memory logic ------------------
    -- regs_in_V
    int_regs_in_V_address0 <= SHIFT_RIGHT(UNSIGNED(regs_in_V_address0), 2)(0 downto 0);
    int_regs_in_V_ce0    <= regs_in_V_ce0;
    int_regs_in_V_we0    <= regs_in_V_we0;
    int_regs_in_V_be0    <= SHIFT_LEFT(TO_UNSIGNED(1, 4), TO_INTEGER(UNSIGNED(regs_in_V_address0(1 downto 0))));
    int_regs_in_V_d0     <= UNSIGNED(regs_in_V_d0) & UNSIGNED(regs_in_V_d0) & UNSIGNED(regs_in_V_d0) & UNSIGNED(regs_in_V_d0);
    ...
    int_regs_in_V_d1     <= UNSIGNED(WDATA);

    process (ACLK)
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_regs_in_V_read <= '0';
            elsif (ACLK_EN = '1') then
                if (ar_hs = '1' and raddr >= ADDR_REGS_IN_V_BASE and raddr <= ADDR_REGS_IN_V_HIGH) then
                    int_regs_in_V_read <= '1';
                else
                    int_regs_in_V_read <= '0';
                end if;
            end if;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (aw_hs = '1' and UNSIGNED(AWADDR(ADDR_BITS-1 downto 0)) >= ADDR_REGS_IN_V_BASE and UNSIGNED(AWADDR(ADDR_BITS-1 downto 0)) <= ADDR_REGS_IN_V_HIGH) then
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (regs_in_V_ce0 = '1') then
                    int_regs_in_V_shift <= UNSIGNED(regs_in_V_address0(1 downto 0));
                end if;
            end if;
        end if;
    end process;
...
entity rpy_to_duty_AXILiteS_s_axi_ram is
    generic (
        BYTES   : INTEGER :=4;
        DEPTH   : INTEGER :=256;
        AWIDTH  : INTEGER :=8);
...
end entity rpy_to_duty_AXILiteS_s_axi_ram;

architecture behave of rpy_to_duty_AXILiteS_s_axi_ram is
