...
//------------------------Parameter----------------------
`define TV_IN_regs_in_V "../tv/cdatafile/c.rpy_to_duty.autotvin_regs_in_V.dat"
parameter ADDR_WIDTH = 5;
parameter DATA_WIDTH = 32;
parameter regs_in_V_DEPTH = 4;
reg [31 : 0] regs_in_V_OPERATE_DEPTH = 0;
parameter regs_in_V_c_bitwidth = 8;
parameter START_ADDR = 0;
parameter rpy_to_duty_continue_addr = 0;
parameter rpy_to_duty_auto_start_addr = 0;
parameter regs_in_V_data_in_addr = 16;
...
reg  BREADY_reg = 0;
reg [DATA_WIDTH - 1 : 0] mem_regs_in_V [regs_in_V_DEPTH - 1 : 0];
reg regs_in_V_write_data_finish;
...
//write regs_in_V reg
reg [31 : 0] write_regs_in_V_count = 0;
reg write_regs_in_V_run_flag = 0;
reg write_one_regs_in_V_data_done = 0;
...
assign TRAN_AXILiteS_write_data_finish = 1 & regs_in_V_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        regs_in_V_write_data_finish <= 0;
        write_regs_in_V_run_flag <= 0; 
        write_regs_in_V_count = 0;
        count_operate_depth_by_bitwidth_and_depth (regs_in_V_c_bitwidth, regs_in_V_DEPTH, regs_in_V_OPERATE_DEPTH);
    end
    else begin
        ...
        if (AESL_ready_reg === 1) begin
            write_regs_in_V_run_flag <= 1; 
            write_regs_in_V_count = 0;
        end
        if (write_one_regs_in_V_data_done === 1) begin
            write_regs_in_V_count = write_regs_in_V_count + 1;
            if (write_regs_in_V_count == regs_in_V_OPERATE_DEPTH) begin
                write_regs_in_V_run_flag <= 0; 
                regs_in_V_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_regs_in_V
    integer write_regs_in_V_resp;
    integer process_num ;
    integer get_ack;
    ...
    integer i;
    integer j;
    reg [31 : 0] regs_in_V_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = regs_in_V_c_bitwidth;
    process_num = 1;
    ...
    while (1) begin
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_regs_in_V_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write regs_in_V data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (regs_in_V_c_bitwidth < 32) begin
                        regs_in_V_data_tmp_reg = mem_regs_in_V[write_regs_in_V_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < regs_in_V_c_bitwidth) begin
                                regs_in_V_data_tmp_reg[j] = mem_regs_in_V[write_regs_in_V_count][i*32 + j];
                            end
                            else begin
                                regs_in_V_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    write (regs_in_V_data_in_addr + write_regs_in_V_count * four_byte_num * 4 + i * 4, regs_in_V_data_tmp_reg, write_regs_in_V_resp);
                end
                process_busy = 0;
                write_one_regs_in_V_data_done <= 1;
                @(posedge clk);
                write_one_regs_in_V_data_done <= 0;
            end   
            process_1_finish <= 1;
        end
        @(posedge clk);
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        ...
        if (write_start_count >= 956) begin
            write_start_run_flag <= 0; 
        end
...
// Read data from file 
initial begin : read_regs_in_V_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [127 : 0] token; 
  reg [127 : 0] token_tmp; 
  //reg [regs_in_V_c_bitwidth - 1 : 0] token_tmp; 
  ...
  count_seperate_factor_by_bitwidth (regs_in_V_c_bitwidth , factor);
  fp = $fopen(`TV_IN_regs_in_V ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_regs_in_V); 
      $finish; 
  end 
  read_token(fp, token); 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < regs_in_V_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ...
          if (factor == 4) begin
              ...
              if (i%factor == 3) begin
                  ...
                  mem_regs_in_V [i/factor] = mem_tmp;
                  mem_tmp [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
