...
`timescale 1ns/1ps
module rpy_to_duty_AXILiteS_s_axi
#(parameter
...
//------------------------Local signal-------------------
    ...
    wire [31:0]                   int_regs_in_V_d1;
    wire [31:0]                   int_regs_in_V_q1;
    reg                           int_regs_in_V_read;
    reg                           int_regs_in_V_write;
    reg  [1:0]                    int_regs_in_V_shift;

//------------------------Instantiation------------------
// int_regs_in_V
rpy_to_duty_AXILiteS_s_axi_ram #(
    .BYTES    ( 4 ),
    .DEPTH    ( 2 )
) int_regs_in_V (
    .clk0     ( ACLK ),
...
assign RVALID  = (rstate == RDDATA) & !int_regs_in_V_read;
...
always @(posedge ACLK) begin
    if (ACLK_EN) begin
        ...
        else if (int_regs_in_V_read) begin
            rdata <= int_regs_in_V_q1;
        end
    end
end
...
//------------------------Memory logic-------------------
// regs_in_V
assign int_regs_in_V_address0 = regs_in_V_address0 >> 2;
...
assign regs_in_V_q0           = int_regs_in_V_q0 >> (int_regs_in_V_shift * 8);
...
assign int_regs_in_V_d1       = WDATA;
// int_regs_in_V_read
always @(posedge ACLK) begin
    if (ARESET)
        int_regs_in_V_read <= 1'b0;
    else if (ACLK_EN) begin
        if (ar_hs && raddr >= ADDR_REGS_IN_V_BASE && raddr <= ADDR_REGS_IN_V_HIGH)
            int_regs_in_V_read <= 1'b1;
        else
            int_regs_in_V_read <= 1'b0;
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (aw_hs && AWADDR[ADDR_BITS-1:0] >= ADDR_REGS_IN_V_BASE && AWADDR[ADDR_BITS-1:0] <= ADDR_REGS_IN_V_HIGH)
...
always @(posedge ACLK) begin
    if (ACLK_EN) begin
        if (regs_in_V_ce0)
            int_regs_in_V_shift <= regs_in_V_address0[1:0];
    end
end
...
module rpy_to_duty_AXILiteS_s_axi_ram
#(parameter
    BYTES  = 4,
    DEPTH  = 256,
    AWIDTH = log2(DEPTH)
