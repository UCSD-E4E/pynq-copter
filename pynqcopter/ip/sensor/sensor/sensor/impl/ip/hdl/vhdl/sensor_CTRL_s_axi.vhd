...
architecture behave of sensor_CTRL_s_axi is
            ...
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_IICSTATUS_O_DATA_0 =>
                        rdata_data <= RESIZE(int_iicStatus_o(31 downto 0), 32);
                    when ADDR_IICSTATUS_O_CTRL =>
                        rdata_data <= (0 => int_iicStatus_o_ap_vld, others => '0');
                    when others =>
                        rdata_data <= (others => '0');
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_IICSTATUS_I_DATA_0) then
                    int_iicStatus_i(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_iicStatus_i(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_iicStatus_o <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (iicStatus_o_ap_vld = '1') then
                    int_iicStatus_o <= UNSIGNED(iicStatus_o); -- clear on read
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_iicStatus_o_ap_vld <= '0';
            elsif (ACLK_EN = '1') then
                if (iicStatus_o_ap_vld = '1') then
                    int_iicStatus_o_ap_vld <= '1';
                elsif (ar_hs = '1' and raddr = ADDR_IICSTATUS_O_CTRL) then
                    int_iicStatus_o_ap_vld <= '0'; -- clear on read
                end if;
            end if;
        end if;
    end process;
